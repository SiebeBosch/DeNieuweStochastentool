...
Public Class clsFrictionDatSTFRRecords

  Friend records As New Dictionary(Of String, clsFrictionDatSTFRRecord)
  ...
  End Sub
    ...
    Friend Sub AddPrefix(ByVal Prefix As String)
        For Each STFR As clsFrictionDatSTFRRecord In records.Values
            STFR.ID = Prefix & STFR.ID
            STFR.ci = Prefix & STFR.ci
        Next
    End Sub

    Public Function GetItem(ID As String) As clsFrictionDatSTFRRecord
    ...
    For Each record As clsFrictionDatSTFRRecord In records.Values
      i += 1
  ...
  Friend Sub Read(ByVal myStrings As Collection)
    Dim i As Integer = 0
    Me.setup.GeneralFunctions.UpdateProgressBar("Reading stucture friction records...", 0, myStrings.Count)
    For Each myString As String In myStrings
      i += 1
      Me.setup.GeneralFunctions.UpdateProgressBar("", i, myStrings.Count)
      Dim myRecord As clsFrictionDatSTFRRecord = New clsFrictionDatSTFRRecord(Me.setup)
