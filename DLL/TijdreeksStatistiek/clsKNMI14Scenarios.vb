...
Public Class clsKNMI14Scenarios
  ...
  Public Sub GL2050()
    Call SUMMER(1, 1.025, 1.025, 1.005)
    Call WINTER(1.014, 1.031, 1.054, 1.054, 0.997)
    Call SPRING(1.0437)
    Call AUTUMN(1.07)
  End Sub
  Public Sub GH2050()
    Call SUMMER(0.81, 1, 1.03, 0.946)
    Call WINTER(1.125, 1.0712, 1, 1.0723, 1.014)
    Call SPRING(1.0216)
    Call AUTUMN(1.08)
  End Sub
  Public Sub WL2050()
    Call SUMMER(1, 1.028, 1.032, 1.007)
    Call WINTER(1.12, 1.07, 1, 1.093, 0.996)
    Call SPRING(1.1077)
    Call AUTUMN(1.03)
  End Sub
  Public Sub WH2050()
    Call SUMMER(0.703, 0.98, 1.105, 0.9)
    Call WINTER(1.325, 1.113, 1, 1.116, 1.0245)
    Call SPRING(1.087)
    Call AUTUMN(1.075)
  End Sub

  Public Sub SUMMER(ByVal Summer01mmVolumeFactor24H As Double, ByVal Summer20mmIncrease24H As Double, ByVal SummerT10VolumeFactor24H As Double, ByVal SummerWetDaysIncrease24H As Double)
    ...
    mySeason.AggregatePrecipitationByDay()
    ...
    For Each myEvent In myDuration.AnnualMaxima.Events.Values
      If myEvent.Sum >= 20 Then
        For ts = myEvent.StartTs To myEvent.StartTs + myDuration.DurationTimesteps - 1
          ...
          Series.Values(ts) *= myMultiplier
        Next
        myEvent.CalculateSum()
      End If
    Next
    ...
    If nTarget < nWetDays Then
      Call RemoveWetDays(nWetDays - nTarget, mySeason)
    ElseIf nTarget > nWetDays Then
      Call AddWetDays(nTarget - nWetDays, mySeason)
    End If
  ...
  End Sub
  Public Sub WINTER(ByVal Winter01mmFactor24H As Double, ByVal Winter10mmFactor24H As Double, ByVal WinterT01VolumeFactor240H As Double, ByVal WinterT10VolumeFactor240H As Double, ByVal WinterWetDaysIncrease24H As Double)
  ...
  End Sub
  Public Sub SPRING(ByVal Factor As Double)
    ...
    For ts = 0 To Series.Dates.Count - 1
      If mySeason.DateInSeason(Series.Dates(ts)) Then
        Series.Values(ts) *= Factor
      End If
    Next

  End Sub
  Public Sub AUTUMN(ByVal Factor As Double)
  ...
  Public Sub RemoveWetDays(ByVal nRemove As Long, ByRef mySeason As clsSeason)
  ...
  End Sub
  Public Sub AddWetDays(ByVal nAdd As Long, ByRef mySeason As clsSeason)
    ...
    For i = 1 To nAdd
      ...
      Series.Values(mySum.StartTs) += 0.1
      mySum.Sum += 0.15
    Next
