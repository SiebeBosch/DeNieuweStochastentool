...
AppId={{e24e4bc2-b089-425b-8ef4-6c4025dd58a1}
AppName={#MyAppName}
...
VersionInfoCopyright=Copyright 2021, Hydroconsult
VersionInfoDescription=De Nieuwe Stochastentool [www.sobek.tools]
...
;write the zip file containing our results viewer
#define viewerdir = "viewer"
Source: "{#SetupLocation}\{#stochastenviewer}"; DestDir: "{app}\{#viewerdir}"
  
[Messages]
BeveledLabel=De Nieuwe Stochastentool by Hydroconsult

[Icons]
;; In start menu:
Name: "{commonstartmenu}\{#MyAppPublisher}\{#MyAppName}"; Filename: "{app}\Stochastentool.exe"; WorkingDir: "{app}"; Comment: "Start De Nieuwe Stochastentool"
;; On desktop:
Name: "{commondesktop}\{#MyAppName}"; Filename: "{app}\Stochastentool.exe"; WorkingDir: "{app}"; Comment: "Start De Nieuwe Stochastentool"
...
;Type: files; Name: "{app}\libecwj2.dll"; Components: MapWinGIS_Core
 ...
 ;result := RegKeyExists(HKEY_LOCAL_MACHINE,'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\app2.exe');
...
function IsNetCDFInstalled: boolean;
begin  result := RegKeyExists (HKEY_LOCAL_MACHINE, 'SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\netCDF 4.8.0');
end;

function InitializeSetup: boolean;
begin
  result := IsNetCDFInstalled;
  if not result then
    MsgBox('You need to install NetCDF before you install De Nieuwe Stochastentool. Install NetCDF from netCDF4.8.0-NC4-64.exe and then run this installer again.', mbError, MB_OK);
end;
